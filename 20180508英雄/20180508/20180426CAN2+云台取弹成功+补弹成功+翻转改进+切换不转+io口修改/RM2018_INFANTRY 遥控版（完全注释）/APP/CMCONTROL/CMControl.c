...
int16_t step=6,step_spin=1,x=0,y=0,m=0,n=0,q=0,p=0;           //step-速度变化率  x-x轴方向变化值 y-y速度变化值; 
int16_t xlimit = XLIMIT_OFFSET,ylimit = YLIMIT_OFFSET;
float ele_dec = 0;
int quick_spin_flag=0;
int quick_spin_step=0;
...
void move(int16_t speedX, int16_t speedY, int16_t rad)
{
		...
		if(quick_spin_flag==1)
		{
			rad *=gears_speedRAD_QE;
		}
		else
		{
		rad *= gears_speedRAD;
	...
	switch(keyboardvalue & (KEY_PRESSED_OFFSET_Q | KEY_PRESSED_OFFSET_E ))
		{
			case ( KEY_PRESSED_OFFSET_Q):
				quick_spin_flag=1;
				p=0;
				if(quick_spin_step>=-30)
				{
					if(quick_spin_step>0)
					{quick_spin_step-=5*step_spin;}
					else if(quick_spin_step<=0)
					{quick_spin_step-=step_spin;}		
				}
				else
				{quick_spin_step=-30;}
				break;
			...
			default:
				...
				if(p>1)
				{
					if(quick_spin_step>0)			{quick_spin_step=quick_spin_step-step;}
					else if(quick_spin_step<0){quick_spin_step=quick_spin_step+step;}
				}
				break;
	  }
