...
uint32_t tG_ms = 500 ;
uint32_t tH_ms = 500 ;
uint32_t tI_ms = 10 ;           //最后一步收回爪子到清空寄存器给的时间
uint32_t tJ_ms = 2000;

uint32_t tA_Init_ms = 0 ;
uint32_t tB_Init_ms = 200 ;          //大约1秒
uint32_t tC_Init_ms = 800 ;
uint32_t tD_Init_ms = 500 ;
uint32_t tE_Init_ms = 1300 ;          //[(8192*3-11500)/(100*0.03)]*2 安全系数
uint32_t tF_Init_ms = 10 ;
uint32_t tG_Init_ms = 500 ;
uint32_t tH_Init_ms = 500 ;
uint32_t tI_Init_ms = 10 ;
uint32_t tJ_Init_ms = 2000;
	  ...
	  if(tH_ms==0&&tI_ms!=0){
		...
		if(tI_ms==0&&tJ_ms!=0){
		  if(setangle1>=FLIFT_POSLIMIT_H){
			setangle1 -= FLIFT_LIFTSPEED*0.12f;
			setangle2 += FLIFT_LIFTSPEED*0.12f;            //这里有修改
		}
			tJ_ms--;
		}
		if(tJ_ms==0){
	    ...
	    case(TASK_A):
			...
			case(TASK_B):
				if(setangle1<=AutoCatch_FLIFT_POSLIMIT_H)
					{
						setangle1 += FLIFT_LIFTSPEED*0.140f;
						setangle2 -= FLIFT_LIFTSPEED*0.140f;            //可能会太快? 
					}
				break;
			case(TASK_C):
				movebackward();
