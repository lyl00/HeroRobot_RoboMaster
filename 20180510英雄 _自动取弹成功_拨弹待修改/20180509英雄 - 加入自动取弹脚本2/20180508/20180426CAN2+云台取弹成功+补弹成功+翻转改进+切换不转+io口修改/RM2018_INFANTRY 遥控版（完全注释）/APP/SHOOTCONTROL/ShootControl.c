...
int blocking_flag=500;
int blocking_target=4000;
int empty_count=100;
...
static int pass_bullet_count=0;
...
void PlatePidInit(void)
{
	...
	SetPIDCR(&MotorPID_plate.Position,(1<<PEN)+(1<<IEN)+(1<<PIDEN)+(1<<INTERVAL)+(1<<DEN));
	PID_Init(&MotorPID_plate.Speed,		PIDVAL_PLATE_SPEED_p, PIDVAL_PLATE_SPEED_i, PIDVAL_PLATE_SPEED_d,-200,200,0);//13 0 0
	SetPIDCR(&MotorPID_plate.Speed,(1<<PEN)+(1<<IEN)+(1<<PIDEN)+(1<<INTERVAL)+(1<<DEN));
}
...
void PlatePID(float SetPosition)
{
	...
	PIDOut_Position_Plate = PID_Driver(4000, NowPosition, &MotorPID_plate.Position , 10000);       //位置环输出
	PIDOut_Speed_Plate = PID_Driver(PIDOut_Position_Plate/20.0f,NowSpeed,&MotorPID_plate.Speed , 10000);//速度环输出
	
	PIDOut_Whole_Shoot = PlateMotorCurrentLegalize(PIDOut_Speed_Plate,5000);  //总输出，幅值限制
...
void Read_Infrared_State(void)
{
	...
	infrared1_state_now=GPIO_ReadInputDataBit(GPIOH,GPIO_Pin_12);
...
int preload_count=0;
int preload_t=100;
void Preload_bullet(void)
{
	switch(RC_Ex_Ctl.key.v & (KEY_PRESSED_OFFSET_SHIFT))
		{ 
		...
		if(preload_now==1&&preload_pre==0)
		{
			preload_count=pass_bullet_count;
			bullet_loaded=0;
		}
//		if(preload_t>0)
//		{
//			preload_t--;
//		}
		if(pass_bullet_count==(preload_count+1))
		{
			bullet_loaded=0;
			//preload_t=1000;
		}		
		preload_pre=preload_now;
}
